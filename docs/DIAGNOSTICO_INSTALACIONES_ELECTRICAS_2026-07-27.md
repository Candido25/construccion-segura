# Diagnóstico de cobertura — instalaciones eléctricas

Versión revisada: `2.5.0`. Parámetros totales: `1519`.
Parámetros relacionados encontrados: `241`.

## A.010 — 9

- `a010-cuarto-residuos-superficies-ventilacion` | Arquitectura y Habitabilidad de la Vivienda | Cuarto de residuos | Condiciones constructivas | 43.2
- `a010-escalera-protegida-cerramiento-resistencia` | Evacuación y protección contra incendios | Caja de escalera protegida | Resistencia al fuego | Artículo 25.3.j
- `a010-escalera-protegida-continuidad` | Evacuación y protección contra incendios | Escalera protegida | Continuidad vertical | Artículo 25.3.b
- `a010-escalera-protegida-puerta-resistencia` | Evacuación y protección contra incendios | Puerta de escalera protegida | Resistencia al fuego | Artículo 25.3.k
- `a010-escalera-protegida-puerta-vano-minimo` | Evacuación y protección contra incendios | Puerta de escalera protegida | Ancho mínimo de vano | Artículo 25.3.f
- `a010-medidor-agua-unidad-independiente` | Arquitectura y Habitabilidad de la Vivienda | Unidad inmobiliaria independiente | Medidor de agua | 48.3
- `a010-retiro-frontal-medidor-electrico-permitido` | Instalaciones exteriores y retiros | Retiro frontal | Murete para medidor eléctrico | 8.3.g
- `a010-retiro-frontal-medidores-gas-permitidos` | Instalaciones exteriores y retiros | Retiro frontal | Reguladores y medidores de gas | 8.3.h
- `a010-retiro-frontal-subestacion-altura-maxima` | Instalaciones exteriores y retiros | Retiro frontal | Subestación eléctrica y ventilación | 8.3.o

## A.020 — 6

- `a020-azotea-baranda-resistencia-puntual` | Azoteas, techos y cubiertas | Baranda de azotea | Resistencia horizontal | 18.4
- `a020-ducto-red-humeda-electricidad-prohibicion` | Impermeabilización, Humedad y Drenaje | Ducto de instalaciones | Separación de redes | Artículo 24
- `a020-medidores-independientes-vivienda` | Arquitectura y Habitabilidad de la Vivienda | Vivienda independiente | Medidores | Artículo 25
- `a020-piso-cocina-resistencia-grasas` | Arquitectura y Habitabilidad de la Vivienda | Piso de cocina | Resistencia a grasas y aceites | Artículo 14
- `a020-piso-exterior-antideslizante` | Arquitectura y Habitabilidad de la Vivienda | Piso exterior | Resistencia al deslizamiento | Artículo 14
- `a020-tension-electrica-vivienda` | Arquitectura y Habitabilidad de la Vivienda | Instalación eléctrica de vivienda | Tensión nominal | Artículo 22

## A.120 — 4

- `a120-bolardos-separacion-altura` | Estacionamientos accesibles | Bolardos entre peatones y vehículos | Separación y altura | Artículo 23.b
- `a120-ingreso-ruta-accesible-desde-acera` | Accesibilidad universal en vivienda | Ingreso de la edificación | Continuidad accesible | Artículo 4.a
- `a120-puerta-doble-hoja-seccion-accesible` | Accesibilidad universal en vivienda | Puerta de dos hojas | Hoja operativa accesible | Artículo 4.b
- `a120-rampa-encuentro-sin-desnivel` | Rampas y rutas accesibles | Encuentro de rampa o escalera | Continuidad superficial | Artículo 6.j

## Buenas prácticas de acabados — revisión técnica — 11

- `criterio-acabado-planos-coordinados` | Control de calidad de acabados | Acabados e instalaciones | Coordinación | sin numeral
- `criterio-contrapiso-juntas-continuidad` | Contrapisos, pisos y enchapes | Contrapiso sobre junta | Continuidad de juntas | sin numeral
- `criterio-drywall-barrera-fuego-completa` | Cielorrasos y sistemas livianos | Sistema con resistencia al fuego | Continuidad del ensamble | sin numeral
- `criterio-drywall-luminarias-soporte` | Cielorrasos y sistemas livianos | Cielorraso | Luminarias y accesorios | sin numeral
- `criterio-enchape-exterior-deslizamiento` | Contrapisos, pisos y enchapes | Piso exterior | Resistencia al deslizamiento | sin numeral
- `criterio-enchape-lote-tono-calibre` | Contrapisos, pisos y enchapes | Piezas de acabado | Lote, tono y calibre | sin numeral
- `criterio-enchape-sonido-hueco-evaluar` | Contrapisos, pisos y enchapes | Enchape terminado | Vacíos o falta de adherencia | sin numeral
- `criterio-pintura-corte-continuidad` | Pinturas y protección de superficies | Paño de pintura | Continuidad de aplicación | sin numeral
- `criterio-tarrajeo-cajas-instalaciones-nivel` | Tarrajeos, revoques y enlucidos | Cajas eléctricas y sanitarias | Coordinación con acabado | sin numeral
- `criterio-tarrajeo-juntas-respetar` | Tarrajeos, revoques y enlucidos | Muro con juntas | Continuidad de juntas | sin numeral
- `criterio-tarrajeo-mortero-compatible` | Tarrajeos, revoques y enlucidos | Tarrajeo | Compatibilidad del mortero | sin numeral

## E.030 — 4

- `e030-diafragma-aberturas-irregularidad` | Diseño sismorresistente | Diafragma de piso o techo | Aberturas que producen irregularidad | Artículos 23-24 y Tabla N.° 12
- `e030-diafragma-rigidez-resistencia-conexion` | Diseño sismorresistente | Diafragma de piso o techo | Verificación para considerarlo rígido | Artículo 30.3-30.6
- `e030-reforzamiento-progresivo` | Evaluación y reforzamiento sísmico | Reforzamiento sísmico progresivo | Criterio de intervención por etapas | 68.4
- `e030-reparacion-rigidez-resistencia-ductilidad` | Evaluación y reforzamiento sísmico | Reparación o reforzamiento | Propiedades estructurales requeridas | 68.1

## E.040 — 10

- `e040-almacenamiento-esfuerzo-termico` | Vidrios, ventanas y cerramientos | Vidrio almacenado | Prevención de tensiones térmicas | 18.3
- `e040-anclaje-resistencia-estructural` | Vidrios, ventanas y cerramientos | Anclaje de sistema vidriado | Resistencia | 13.1
- `e040-baranda-vidrio-calculo-sobrecarga` | Vidrios, ventanas y cerramientos | Baranda o parapeto de vidrio | Cálculo por sobrecarga horizontal | 5.12
- `e040-barrera-fuego-humo-resistencia` | Vidrios, ventanas y cerramientos | Encuentro de sistema vidriado con pisos o sectores | Barrera de fuego y humo | 16
- `e040-cables-tensores-calculo` | Vidrios, ventanas y cerramientos | Cable o tensor de soporte | Dimensionamiento | 17.2
- `e040-cables-tensores-inoxidable` | Vidrios, ventanas y cerramientos | Cable o tensor de soporte | Material | 17.1
- `e040-insulado-cada-hoja-cumplimiento` | Vidrios, ventanas y cerramientos | Vidrio insulado | Verificación de cada hoja | 11.2
- `e040-piso-vidrio-carga-uniforme` | Vidrios, ventanas y cerramientos | Superficie transitable de vidrio | Carga vertical uniformemente distribuida | 5.13.a
- `e040-puerta-mampara-zona-impacto-lateral` | Vidrios, ventanas y cerramientos | Paño fijo junto a puerta o mampara | Extensión lateral de zona de impacto | 6.2.c
- `e040-silicona-estructural-proyecto` | Vidrios, ventanas y cerramientos | Silicona estructural | Selección del producto | 14.1

## E.050 — 2

- `e050-calzadura-concreto-ciclopeo` | Excavaciones, sostenimiento y calzaduras | Paño de calzadura | Concreto ciclópeo y resistencia | 39.12.7.c
- `e050-its-condiciones-aplicacion` | Terreno y estudio de suelos | Informe Técnico de Suelos | Condiciones del supuesto simplificado | 5.36 y 6.3.1-6.3.4

## E.060 — 22

- `e060-aceptacion-promedio-tres-ensayos` | Control de calidad del concreto | Concreto ensayado | Promedio de resistencia | 5.6.3.3(a)
- `e060-aceptacion-resultado-individual` | Control de calidad del concreto | Resultado individual de resistencia | Reducción máxima respecto a f'c | 5.6.3.3(b)
- `e060-colocacion-plastica-continua` | Procesos de concreto | Vaciado de concreto | Continuidad y estado plástico | 5.10.2 y 5.10.5
- `e060-columna-tuberias-area-maxima` | Instalaciones embebidas | Tuberías embebidas en columna | Área ocupada | 6.3.4
- `e060-concreto-estructural-resistencia-minima` | Concreto y acero de refuerzo | Concreto estructural | Resistencia especificada a compresión | 5.1.1 y 9.4.1
- `e060-curado-alta-resistencia-inicial-minimo` | Concreto y acero de refuerzo | Concreto de alta resistencia inicial | Duración del curado húmedo | 5.11.6
- `e060-curado-obra-relacion-minima-laboratorio` | Control de calidad del concreto | Probetas curadas en obra | Relación con probetas de laboratorio | 5.6.4.3
- `e060-desencofrado-sin-plazo-universal` | Encofrados y apuntalamiento | Desencofrado | Momento de retiro | 6.2.1
- `e060-empalme-traccion-clase-a-longitud` | Concreto y acero de refuerzo | Empalme por traslape en tracción Clase A | Longitud mínima | 12.15.1-12.15.2 y Tabla 12.3
- `e060-empalme-traccion-clase-b-longitud` | Concreto y acero de refuerzo | Empalme por traslape en tracción Clase B | Longitud mínima | 12.15.1-12.15.2 y Tabla 12.3
- `e060-ensayo-resistencia-dos-probetas` | Control de calidad del concreto | Ensayo de resistencia | Probetas por resultado | 5.6.2.3
- `e060-ensayo-resistencia-frecuencia` | Control de calidad del concreto | Muestreo de concreto | Frecuencia mínima | 5.6.2.1
- `e060-escalera-garganta-sin-minimo-universal` | Escaleras | Losa de escalera de concreto armado | Espesor de garganta | 9.6.1, 9.6.2.1 y Tabla 9.1
- `e060-estructura-sin-puntales-carga-maxima` | Encofrados y apuntalamiento | Estructura sin apuntalamiento | Carga de construcción | 6.2.2.2
- `e060-evaluacion-acero-resistencia` | Evaluación de estructuras de concreto | Material existente | Determinación de resistencia del acero | 20.2.4
- `e060-evaluacion-concreto-resistencia` | Evaluación de estructuras de concreto | Material existente | Determinación de resistencia del concreto | 20.2.3
- `e060-evaluacion-estructura-metodos` | Evaluación de estructuras de concreto | Estructura existente de concreto | Métodos de evaluación de resistencia | 20.1.1
- `e060-prueba-carga-fisuras-anclaje-investigar` | Evaluación de estructuras de concreto | Aceptación de prueba de carga | Fisuras en anclajes o empalmes | 20.5.5
- `e060-testigos-cantidad-por-resultado-bajo` | Control de calidad del concreto | Núcleos o testigos de concreto | Cantidad por resultado bajo | 5.6.5.2
- `e060-transporte-sin-interrupciones` | Procesos de concreto | Abastecimiento de concreto | Continuidad del suministro | 5.9.2
- `e060-viga-sismica-muros-dual-empalme-zona-prohibida` | Vigas y losas de concreto armado | Viga del sistema sismorresistente con muros o dual tipo I | Zona sin empalmes traslapados | 21.4.4.2
- `e060-vigueta-barra-inferior-continuidad` | Vigas y losas de concreto armado | Vigueta construida en obra | Continuidad de barra inferior | 7.13.2.1

## E.070 — 2

- `e070-confinamiento-concreto-resistencia-minima` | Albañilería confinada y armada | Columna y viga de confinamiento | Resistencia del concreto | 9.1, 11.6 y 20.1(f)
- `e070-grout-resistencia-minima` | Albañilería confinada y armada | Concreto líquido o grout | Resistencia a compresión | 7.5

## EM.010 — 36

- `em010-alumbrado-niveles-normativos` | Instalaciones eléctricas en ejecución | Iluminación | Niveles de alumbrado | Artículo 6
- `em010-ambiente-humedo-proteccion` | Instalaciones eléctricas en ejecución | Instalación en zona húmeda | Grado de protección | Artículo 11
- `em010-cajas-accesibles-no-ocultas` | Instalaciones eléctricas en ejecución | Cajas de paso | Accesibilidad | Artículo 10
- `em010-calculo-iluminacion-obligatorio` | Instalaciones Eléctricas en Viviendas | Sistema de iluminación | Cálculo del alumbrado | Artículo 6
- `em010-canalizacion-material-aprobado` | Instalaciones eléctricas en ejecución | Canalizaciones | Materiales | Artículo 10
- `em010-carga-calculo-demanda` | Instalaciones eléctricas en ejecución | Carga instalada | Cálculo de demanda | Artículo 5
- `em010-circuitos-independientes-cargas` | Instalaciones eléctricas en ejecución | Circuitos derivados | Separación de cargas | Artículo 9
- `em010-cne-utilizacion-obligatorio` | Instalaciones eléctricas en ejecución | Instalación interior | Código aplicable | Artículo 3
- `em010-conductores-material-certificado` | Instalaciones eléctricas en ejecución | Conductores | Conformidad | Artículo 10
- `em010-construccion-etapas-reservas` | Instalaciones Eléctricas en Viviendas | Instalación eléctrica de construcción progresiva | Proyecto y reservas | Artículo 9
- `em010-coordinacion-otras-instalaciones` | Instalaciones eléctricas en ejecución | Coordinación de obra | Interferencias | Artículo 12
- `em010-ejecucion-personal-calificado` | Instalaciones Eléctricas en Viviendas | Instalación eléctrica interior | Personal de ejecución | Artículo 5
- `em010-emergencia-rutas-multifamiliar` | Instalaciones Eléctricas en Viviendas | Ruta de evacuación en vivienda multifamiliar | Alumbrado de emergencia | Artículo 11
- `em010-emergencia-segun-uso` | Instalaciones eléctricas en ejecución | Iluminación de emergencia | Aplicación | Artículo 7
- `em010-expediente-conforme-obra` | Instalaciones eléctricas en ejecución | Documentación final | Conforme a obra | Artículo 13
- `em010-iluminacion-sin-parpadeo` | Instalaciones Eléctricas en Viviendas | Sistema de iluminación | Parpadeo y efecto estroboscópico | Artículo 6
- `em010-iluminancia-bano-minima` | Instalaciones Eléctricas en Viviendas | Baño | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-circulacion-comun-minima` | Instalaciones Eléctricas en Viviendas | Circulación común | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-cocina-minima` | Instalaciones Eléctricas en Viviendas | Cocina | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-comedor-minima` | Instalaciones Eléctricas en Viviendas | Comedor | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-dormitorio-minima` | Instalaciones Eléctricas en Viviendas | Dormitorio | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-escalera-comun-minima` | Instalaciones Eléctricas en Viviendas | Escalera común | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-espejo-bano-minima` | Instalaciones Eléctricas en Viviendas | Zona de espejo del baño | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-estacionamiento-cubierto-minima` | Instalaciones Eléctricas en Viviendas | Estacionamiento cubierto | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-estudio-lavanderia-trabajo-minima` | Instalaciones Eléctricas en Viviendas | Estudio, lavandería o zona de trabajo | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-frente-ascensor-minima` | Instalaciones Eléctricas en Viviendas | Frente de ascensor | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-patio-abierto-minima` | Instalaciones Eléctricas en Viviendas | Patio o área abierta | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-iluminancia-sala-minima` | Instalaciones Eléctricas en Viviendas | Sala | Iluminancia mantenida mínima | Artículo 11, tabla de niveles de iluminación
- `em010-planos-especificaciones-obligatorios` | Instalaciones eléctricas en ejecución | Documentación | Planos y especificaciones | Artículo 4
- `em010-proteccion-contacto-directo` | Instalaciones eléctricas en ejecución | Protección eléctrica | Contacto directo | Artículo 8
- `em010-proteccion-contacto-indirecto` | Instalaciones eléctricas en ejecución | Protección eléctrica | Contacto indirecto | Artículo 8
- `em010-proyecto-profesional-responsable` | Instalaciones eléctricas en ejecución | Proyecto eléctrico | Responsabilidad profesional | Artículo 2
- `em010-pruebas-finales-instalacion` | Instalaciones Eléctricas en Viviendas | Instalación eléctrica terminada | Pruebas y documentación final | Artículo 8
- `em010-pruebas-previas-energizacion` | Instalaciones eléctricas en ejecución | Pruebas eléctricas | Recepción | Artículo 13
- `em010-puesta-tierra-obligatoria` | Instalaciones eléctricas en ejecución | Protección eléctrica | Puesta a tierra | Artículo 8
- `em010-tablero-accesible-identificado` | Instalaciones eléctricas en ejecución | Tablero eléctrico | Accesibilidad e identificación | Artículo 9

## EM.020 — 25

- `em020-acometida-canalizaciones-minimas` | Instalaciones Eléctricas en Viviendas | Ingreso de telecomunicaciones | Cantidad de ductos | Artículo 10
- `em020-alimentacion-conductor-minimo` | Instalaciones Eléctricas en Viviendas | Alimentación eléctrica del cuarto de telecomunicaciones | Sección del conductor | Artículo 13
- `em020-bandeja-distancia-techo` | Instalaciones Eléctricas en Viviendas | Bandeja en cuarto de telecomunicaciones | Separación al techo | Artículo 12
- `em020-caja-horizontal-separacion-maxima` | Instalaciones Eléctricas en Viviendas | Caja de paso en canalización horizontal de usuario | Separación máxima | Artículo 17
- `em020-caja-paso-empotrada-separacion` | Instalaciones Eléctricas en Viviendas | Caja de paso en canalización empotrada | Separación máxima | Artículo 21
- `em020-caja-paso-subterranea-separacion` | Instalaciones Eléctricas en Viviendas | Caja de paso en canalización subterránea | Separación máxima | Artículo 21
- `em020-caja-terminal-volumen-minimo` | Instalaciones Eléctricas en Viviendas | Caja terminal de telecomunicaciones | Dimensiones o volumen | Artículo 19
- `em020-camara-entrada-prohibicion-empalmes` | Instalaciones Eléctricas en Viviendas | Cámara de entrada | Empalmes y equipos | Artículo 11
- `em020-canalizacion-curvas-maximas` | Instalaciones Eléctricas en Viviendas | Tramo de canalización de telecomunicaciones | Curvas de 90° entre cajas | Artículo 17
- `em020-circuito-iluminacion-proteccion` | Instalaciones Eléctricas en Viviendas | Circuito de iluminación del cuarto de telecomunicaciones | Protección | Artículo 13
- `em020-circuito-tomacorrientes-proteccion` | Instalaciones Eléctricas en Viviendas | Circuito de tomacorrientes del cuarto de telecomunicaciones | Protección | Artículo 13
- `em020-conducto-entrada-diametro-minimo` | Instalaciones Eléctricas en Viviendas | Conducto desde ingreso al cuarto de telecomunicaciones | Diámetro | Artículo 20
- `em020-cuarto-iluminancia-minima` | Instalaciones Eléctricas en Viviendas | Cuarto de telecomunicaciones | Iluminancia | Artículo 14
- `em020-cuarto-telecom-espacio-frontal` | Instalaciones Eléctricas en Viviendas | Cuarto de telecomunicaciones | Espacio libre frente a gabinetes | Artículo 12
- `em020-cuarto-telecom-separacion-equipos` | Instalaciones Eléctricas en Viviendas | Cuarto de telecomunicaciones | Separación de salas con interferencia o riesgo | Artículo 13
- `em020-gabinete-complementario-dimensiones` | Instalaciones Eléctricas en Viviendas | Gabinete complementario | Dimensiones mínimas | Artículo 16
- `em020-gabinete-secundario-area-servida` | Instalaciones Eléctricas en Viviendas | Gabinete secundario | Área máxima servida | Artículo 16
- `em020-gabinete-vertical-separacion-maxima` | Instalaciones Eléctricas en Viviendas | Gabinetes en canalización vertical | Separación máxima | Artículo 16
- `em020-pau-altura-instalacion` | Instalaciones Eléctricas en Viviendas | Punto de Acceso al Usuario | Altura de instalación | Artículo 18
- `em020-pau-caja-dimensiones-minimas` | Instalaciones Eléctricas en Viviendas | Caja del Punto de Acceso al Usuario | Dimensiones mínimas | Artículo 18
- `em020-pau-tomas-minimas` | Instalaciones Eléctricas en Viviendas | Punto de Acceso al Usuario | Cantidad de tomas | Artículo 18
- `em020-proteccion-diferencial` | Instalaciones Eléctricas en Viviendas | Tablero del cuarto de telecomunicaciones | Protección diferencial | Artículo 13
- `em020-proteccion-general` | Instalaciones Eléctricas en Viviendas | Tablero del cuarto de telecomunicaciones | Interruptor general | Artículo 13
- `em020-tablero-reserva-minima` | Instalaciones Eléctricas en Viviendas | Tablero de telecomunicaciones | Reserva de capacidad | Artículo 13
- `em020-toma-electrica-cerca-terminal` | Instalaciones Eléctricas en Viviendas | Caja terminal de telecomunicaciones | Distancia a tomacorriente | Artículo 19

## EM.040 — 1

- `em040-rejilla-resistencia-mecanica` | Instalaciones de Gas en Viviendas | Rejilla de ventilación para gas | Resistencia | 8.3(c)(v)

## EM.060 — 2

- `em060-caja-fuego-espesor-total-minimo` | Chimeneas, hogares y climatización | Caja de fuego con forro | Espesor total | Artículo 4.2
- `em060-caja-fuego-sin-forro-espesor` | Chimeneas, hogares y climatización | Caja de fuego sin forro | Espesor de muros | Artículo 4.2

## EM.070 — 8

- `em070-cabina-boton-alarma` | Transporte mecánico y equipos especiales | Cabina de ascensor | Botón de alarma | 5.5.a
- `em070-instalaciones-garantizadas-fabricante` | Transporte mecánico y equipos especiales | Instalaciones del equipo de transporte vertical | Responsabilidad del fabricante | 5.8.e
- `em070-pozo-elementos-ajenos-prohibidos` | Transporte mecánico y equipos especiales | Pozo de ascensor | Instalaciones ajenas | 6.7.l
- `em070-pozo-fuego-hasta-3-pisos` | Ascensores y Transporte Mecánico | Pozo de ascensor en edificación de hasta tres pisos | Resistencia al fuego | 6.7(g)
- `em070-pozo-resistencia-fuego` | Transporte mecánico y equipos especiales | Pozo de ascensor en edificación de más de tres pisos | Resistencia al fuego del cerramiento | 6.7(g)
- `em070-proyecto-tecnico-multidisciplinario` | Ascensores y Transporte Mecánico | Equipo de transporte vertical | Contenido del proyecto técnico | 6.2
- `em070-puerta-pozo-resistencia-relativa` | Transporte mecánico y equipos especiales | Puerta de acceso al pozo | Resistencia al fuego relativa | 6.7.g
- `em070-tablero-independiente-por-equipo` | Transporte mecánico y equipos especiales | Instalación eléctrica de ascensor | Tablero independiente | 5.8.b

## EM.080 — 12

- `em080-fv-bateria-prevenir-cortocircuito` | Energía Solar en Viviendas | Terminales de batería solar | Cortocircuito accidental | 6.2.2
- `em080-fv-blindaje-cable-conector` | Energía Solar en Viviendas | Blindaje mecánico de cables | Terminación de bordes | 6.2.6
- `em080-fv-cables-cne` | Energía Solar en Viviendas | Cableado fotovoltaico | Cumplimiento eléctrico | 6.2.2
- `em080-fv-cables-fijados` | Energía Solar en Viviendas | Cableado fotovoltaico | Sujeción | 6.2.2
- `em080-fv-marcos-puesta-tierra` | Energía Solar en Viviendas | Marcos metálicos de paneles | Puesta a tierra | 6.2.6
- `em080-fv-protecciones-electricas` | Energía Solar en Viviendas | Instalación fotovoltaica | Protecciones eléctricas | 6.2.6
- `em080-fv-proveedor-datos-electricos` | Energía Solar en Viviendas | Módulo fotovoltaico | Datos eléctricos del proveedor | 6.2.1
- `em080-fv-soporte-facilita-limpieza` | Energía Solar en Viviendas | Estructura de paneles | Acceso a módulos y cajas | 6.2.4
- `em080-fv-tierra-no-pararrayo` | Energía Solar en Viviendas | Puesta a tierra de paneles | Función | 6.2.6
- `em080-fv-verificar-superficie-soporte` | Energía Solar en Viviendas | Paneles sobre cubierta | Resistencia del soporte | 6.2.5
- `em080-terma-partes-metalicas-puesta-tierra` | Energía Solar en Viviendas | Partes metálicas electrificadas de terma solar | Puesta a tierra | 6.1.5
- `em080-terma-solar-verificacion-carga` | Energía Solar en Viviendas | Terma solar sobre techo o terraza | Verificación estructural | 6.1.1 y 6.1.4

## Evaluación profesional de edificaciones existentes — 2

- `criterio-abrir-vano-evaluar-camino-cargas` | Diagnóstico, mantenimiento y reparación | Remodelación estructural | Apertura o ampliación de vanos | Aplicación profesional caso por caso
- `criterio-reparacion-material-compatible` | Diagnóstico, mantenimiento y reparación | Materiales de reparación | Compatibilidad y preparación | Aplicación profesional caso por caso

## Formación técnica SENCICO — 60

- `criterio-electrico-acta-pruebas` | Pruebas y recepción eléctrica | Entrega | Registro | Aplicación práctica supervisada
- `criterio-electrico-apriete-torque` | Tableros y protecciones | Bornes | Ajuste | Aplicación práctica supervisada
- `criterio-electrico-balance-fases` | Tableros y protecciones | Circuitos | Balance | Aplicación práctica supervisada
- `criterio-electrico-bano-grado-proteccion` | Accesorios y luminarias | Baño y exterior | Protección | Aplicación práctica supervisada
- `criterio-electrico-breaker-no-sobredimensionar` | Tableros y protecciones | Interruptor automático | Calibre | Aplicación práctica supervisada
- `criterio-electrico-cajas-a-plomo` | Canalizaciones eléctricas | Cajas empotradas | Alineación | Aplicación práctica supervisada
- `criterio-electrico-cajas-tapadas` | Canalizaciones eléctricas | Cajas durante obra | Protección | Aplicación práctica supervisada
- `criterio-electrico-canalizacion-continuidad` | Canalizaciones eléctricas | Conduit | Continuidad | Aplicación práctica supervisada
- `criterio-electrico-conector-aprobado` | Conductores y empalmes | Empalme | Conectores | Aplicación práctica supervisada
- `criterio-electrico-curvas-sin-estrangular` | Canalizaciones eléctricas | Tubería conduit | Curvas | Aplicación práctica supervisada
- `criterio-electrico-diferencial-probar` | Tableros y protecciones | Interruptor diferencial | Prueba | Aplicación práctica supervisada
- `criterio-electrico-electrodo-accesible` | Puesta a tierra | Pozo o electrodo | Inspección | Aplicación práctica supervisada
- `criterio-electrico-empalme-en-caja` | Conductores y empalmes | Caja de derivación | Ubicación | Aplicación práctica supervisada
- `criterio-electrico-energizacion-controlada` | Pruebas y recepción eléctrica | Puesta en servicio | Energización | Aplicación práctica supervisada
- `criterio-electrico-equipotencial-metal` | Puesta a tierra | Masas metálicas | Unión equipotencial | Aplicación práctica supervisada
- `criterio-electrico-fotos-ocultos` | Pruebas y recepción eléctrica | Trabajos ocultos | Registro | Aplicación práctica supervisada
- `criterio-electrico-guia-canalizacion` | Canalizaciones eléctricas | Tuberías vacías | Guía | Aplicación práctica supervisada
- `criterio-electrico-identificar-conductores` | Conductores y empalmes | Conductores | Identificación | Aplicación práctica supervisada
- `criterio-electrico-limpiar-antes-cablear` | Canalizaciones eléctricas | Tubería | Limpieza | Aplicación práctica supervisada
- `criterio-electrico-llenado-canalizacion` | Conductores y empalmes | Tubería | Ocupación | Aplicación práctica supervisada
- `criterio-electrico-lubricante-compatible` | Conductores y empalmes | Tendido | Lubricación | Aplicación práctica supervisada
- `criterio-electrico-luminaria-calor` | Accesorios y luminarias | Luminaria | Temperatura | Aplicación práctica supervisada
- `criterio-electrico-luminaria-soporte` | Accesorios y luminarias | Luminaria | Soporte | Aplicación práctica supervisada
- `criterio-electrico-no-cortar-acero` | Canalizaciones eléctricas | Estructura | Interferencia con acero | Aplicación práctica supervisada
- `criterio-electrico-no-danar-aislamiento` | Conductores y empalmes | Cableado | Aislamiento | Aplicación práctica supervisada
- `criterio-electrico-no-doble-conductor` | Tableros y protecciones | Borne | Número de conductores | Aplicación práctica supervisada
- `criterio-electrico-no-empalme-tuberia` | Conductores y empalmes | Canalización | Empalmes ocultos | Aplicación práctica supervisada
- `criterio-electrico-no-extension-permanente` | Accesorios y luminarias | Alimentación | Extensiones | Aplicación práctica supervisada
- `criterio-electrico-no-retorcido-solo` | Conductores y empalmes | Empalme | Método | Aplicación práctica supervisada
- `criterio-electrico-pases-previstos` | Canalizaciones eléctricas | Muros y losas | Pases | Aplicación práctica supervisada
- `criterio-electrico-placa-cubre-caja` | Accesorios y luminarias | Placa | Cobertura | Aplicación práctica supervisada
- `criterio-electrico-plano-conforme-obra` | Pruebas y recepción eléctrica | Documentación | Conforme a obra | Aplicación práctica supervisada
- `criterio-electrico-prueba-aislamiento` | Pruebas y recepción eléctrica | Conductores | Aislamiento | Aplicación práctica supervisada
- `criterio-electrico-prueba-caida-tension` | Pruebas y recepción eléctrica | Carga | Caída de tensión | Aplicación práctica supervisada
- `criterio-electrico-prueba-continuidad` | Pruebas y recepción eléctrica | Circuitos | Continuidad | Aplicación práctica supervisada
- `criterio-electrico-prueba-polaridad` | Pruebas y recepción eléctrica | Salidas | Polaridad | Aplicación práctica supervisada
- `criterio-electrico-prueba-protecciones` | Pruebas y recepción eléctrica | Protecciones | Funcionamiento | Aplicación práctica supervisada
- `criterio-electrico-prueba-tension` | Pruebas y recepción eléctrica | Circuitos | Tensión | Aplicación práctica supervisada
- `criterio-electrico-replanteo-puntos` | Canalizaciones eléctricas | Replanteo | Ubicación de puntos | Aplicación práctica supervisada
- `criterio-electrico-reserva-cajas` | Conductores y empalmes | Caja | Longitud de reserva | Aplicación práctica supervisada
- `criterio-electrico-reserva-tablero` | Tableros y protecciones | Tablero | Capacidad futura | Aplicación práctica supervisada
- `criterio-electrico-seccion-proyecto` | Conductores y empalmes | Conductor | Sección | Aplicación práctica supervisada
- `criterio-electrico-separar-energia-datos` | Canalizaciones eléctricas | Energía y comunicaciones | Separación | Aplicación práctica supervisada
- `criterio-electrico-tablero-directorio` | Tableros y protecciones | Tablero | Directorio | Aplicación práctica supervisada
- `criterio-electrico-tablero-firme` | Tableros y protecciones | Tablero | Fijación | Aplicación práctica supervisada
- `criterio-electrico-tablero-neutro-tierra` | Tableros y protecciones | Barras | Separación | Aplicación práctica supervisada
- `criterio-electrico-tablero-ubicacion-seca` | Tableros y protecciones | Tablero | Ubicación | Aplicación práctica supervisada
- `criterio-electrico-termografia-tablero` | Pruebas y recepción eléctrica | Tablero | Calentamiento | Aplicación práctica supervisada
- `criterio-electrico-tierra-continuidad` | Puesta a tierra | Conductor de protección | Continuidad | Aplicación práctica supervisada
- `criterio-electrico-tierra-medir` | Puesta a tierra | Sistema | Medición | Aplicación práctica supervisada
- `criterio-electrico-tierra-no-interrumpida` | Puesta a tierra | Conductor de protección | Integridad | Aplicación práctica supervisada
- `criterio-electrico-tierra-uniones-corrosion` | Puesta a tierra | Conexiones | Corrosión | Aplicación práctica supervisada
- `criterio-electrico-tomacorriente-firme` | Accesorios y luminarias | Tomacorriente | Fijación | Aplicación práctica supervisada
- `criterio-electrico-tomacorriente-polaridad` | Accesorios y luminarias | Tomacorriente | Polaridad | Aplicación práctica supervisada
- `criterio-sanitaria-bomba-no-trabajar-seco` | Ejecución y control de instalaciones sanitarias | Equipo de bombeo | Protección | sin numeral
- `criterio-sanitaria-plano-conforme-obra` | Ejecución y control de instalaciones sanitarias | Recepción sanitaria | Plano conforme a obra | sin numeral
- `criterio-sanitaria-sellar-pasamuros` | Ejecución y control de instalaciones sanitarias | Paso de tuberías | Sellado | sin numeral
- `criterio-sanitaria-valvulas-accesibles` | Ejecución y control de instalaciones sanitarias | Válvulas de agua | Accesibilidad | sin numeral
- `criterio-sanitaria-ventilacion-no-taponar` | Ejecución y control de instalaciones sanitarias | Ventilación sanitaria | Continuidad | sin numeral
- `criterio-sanitaria-verificar-registros-accesibles` | Ejecución y control de instalaciones sanitarias | Recepción sanitaria | Accesibilidad de registros | sin numeral

## G.050 — 19

- `g050-cables-mangueras-no-cruzar-rutas` | Orden, limpieza y residuos de obra | Cables y mangueras | Tendido fuera de tránsito | 15
- `g050-caida-anclaje-resistencia-minima` | Seguridad durante la construcción | Punto de anclaje de sistema anticaídas | Resistencia mínima | 20.1
- `g050-conductor-provisional-agua-humedad` | Obras preliminares e instalaciones provisionales | Conductor eléctrico provisional | Protección frente al agua y humedad | 7.3
- `g050-electricidad-ambiente-inflamable-explosion` | Obras preliminares e instalaciones provisionales | Instalación eléctrica en ambiente inflamable | Protección contra explosión | 7.3
- `g050-electricidad-provisional-intemperie-lluvia` | Obras preliminares e instalaciones provisionales | Instalación eléctrica provisional | Protección en zonas lluviosas | 7.3
- `g050-electricidad-provisional-persona-competente` | Seguridad durante la construcción | Instalación eléctrica provisional | Responsable de ejecución y mantenimiento | 7.3
- `g050-electricidad-provisional-puesta-tierra` | Seguridad durante la construcción | Circuitos eléctricos provisionales | Puesta a tierra | 7.3
- `g050-enchufe-industrial-blindado` | Obras preliminares e instalaciones provisionales | Enchufe y tomacorriente provisional | Tipo industrial sellado | 7.3
- `g050-evacuacion-rutas-libres-zona-segura` | Circulación, señalización y protección de terceros | Ruta de evacuación | Continuidad y destino | 7.6
- `g050-excavacion-mecanica-servicios-activos` | Excavaciones, zanjas y protección de colindantes | Excavación mecánica cerca de servicios | Desenergización y cierre | 23.1
- `g050-excavacion-servicio-encontrado-suspender` | Excavaciones, zanjas y protección de colindantes | Servicio enterrado no identificado | Suspensión del trabajo | 23.1
- `g050-extension-electrica-calibre-minimo` | Seguridad durante la construcción | Extensión eléctrica temporal | Conductor mínimo | 7.3
- `g050-extension-electrica-empalme-maximo` | Seguridad durante la construcción | Extensión eléctrica temporal | Frecuencia de empalmes | 7.3
- `g050-extension-no-cruzar-transito` | Obras preliminares e instalaciones provisionales | Extensión eléctrica temporal | Cruce de tránsito y exposición mecánica | 7.3
- `g050-luminaria-portatil-antichoque` | Obras preliminares e instalaciones provisionales | Punto de iluminación portátil | Protección y ubicación | 7.8
- `g050-obra-cerco-perimetral-accesos` | Circulación, señalización y protección de terceros | Cerco perimetral de obra | Aislamiento y accesos diferenciados | 7.4
- `g050-proteccion-colectiva-memoria-planos` | Planificación y protecciones de seguridad | Diseño de protección colectiva | Sustento técnico | 14
- `g050-tablero-diferenciales-sensibilidad` | Seguridad durante la construcción | Tablero eléctrico provisional | Interruptores diferenciales | 7.3
- `g050-tablero-provisional-gabinete-seguro` | Obras preliminares e instalaciones provisionales | Tablero eléctrico provisional | Gabinete, cierre y puesta a tierra | 7.3

## GE.040 — 1

- `ge040-equipo-vibracion-aislamiento` | Uso, mantenimiento y conservación | Equipos y maquinaria | Aislamiento de vibraciones | Artículo 8

## IS.010 — 4

- `is010-caja-registro-cambios-red` | Instalaciones sanitarias en ejecución | Red exterior de desagüe | Caja en cambios de trazado | 6.2.k
- `is010-caja-registro-tramo-recto-maximo` | Instalaciones Sanitarias en Viviendas | Cajas de registro en tramo recto | Separación máxima | 6.2
- `is010-deposito-obligatorio-abastecimiento-discontinuo` | Instalaciones sanitarias en ejecución | Almacenamiento de agua | Obligación por discontinuidad o baja presión | 2.4.b
- `is010-registro-pie-montante` | Instalaciones sanitarias en ejecución | Montante de desagüe | Registro al pie | 6.2.j

## Licencia de edificación — modalidad B — 1

- `licencia-planos-especialidades` | Licencias y alcance de intervenciones | Documentación de licencia | Planos de especialidades | Supuestos, vigencia y requisitos publicados en gob.pe
