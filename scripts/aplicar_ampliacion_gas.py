...
def main():
 ...
 esperados=(1604+K,3067+K,1230+NV,356+NC)
 ...
 s=C.read_text(encoding='utf-8'); s=rep(s,'if len(base.parametros) < 1604:',f'if len(base.parametros) < {1604+K}:'); s=rep(s,'por lo menos 1604 parámetros revisados',f'por lo menos {1604+K} parámetros revisados'); s=rep(s,'if validados < 1230:',f'if validados < {1230+NV}:'); s=rep(s,'al menos 1230 numerales RNE validados',f'al menos {1230+NV} numerales RNE validados'); s=rep(s,'if base.version != "2.6.0":','if base.version != "2.7.0":'); s=rep(s,'La versión normativa esperada es 2.6.0','La versión normativa esperada es 2.7.0'); s=rep(s,'if criterios < 356:',f'if criterios < {356+NC}:'); s=rep(s,'al menos 356 criterios técnicos revisados',f'al menos {356+NC} criterios técnicos revisados'); s=rep(s,'if len(todas) < 3067:',f'if len(todas) < {3067+K}:'); s=rep(s,'por lo menos 3067 preguntas técnicas',f'por lo menos {3067+K} preguntas técnicas')
