(() => {
  ...
  const HERO_BRAND_SOURCE = "/assets/brand/portada-principal-construccion-segura.webp?v=20260714-4";
  const HERO_BRAND_FALLBACK = "/assets/site-photos/web/hero-portada.webp?v=20260714-4";
  ...
  const buildRepairedHeroUrl = async () => {
    const response = await fetch(HERO_BRAND_SOURCE, { cache: "no-store" });
    if (!response.ok) {
  ...
  const setHeroBackground = (url, ready = false) => {
    document.querySelectorAll(".hero-media-brand .hero-image").forEach((heroImage) => {
      heroImage.style.backgroundImage = `linear-gradient(200deg, rgba(255, 255, 255, 0.01), rgba(20, 34, 42, 0.08)), url("${url}")`;
      ...
      if (ready) heroImage.dataset.heroBrandReady = "true";
    });
  };

  const applyHeroBrandImage = () => {
    if (!document.querySelector(".hero-media-brand .hero-image")) return;

    setHeroBackground(HERO_BRAND_FALLBACK);
    heroBrandLoadPromise ||= buildRepairedHeroUrl();
    heroBrandLoadPromise
      .then((imageUrl) => setHeroBackground(imageUrl, true))
      .catch((error) => {
