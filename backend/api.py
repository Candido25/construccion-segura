...
# Carga la clave secreta desde tu archivo .env
load_dotenv()
...
def consultar_asistente(pregunta_del_usuario):
    # 1. Lee tu base de preguntas técnicas de construccionsegura.org.pe
    ruta_json = os.path.join(os.path.dirname(__file__), "preguntas_tecnicas.json")
    
    with open(ruta_json, "r", encoding="utf-8") as archivo:
        base_conocimiento = json.load(archivo)
    
    # 2. Crea las reglas del asistente en español
    instrucciones_sistema = (
        "Eres el asistente técnico oficial de construccionsegura.org.pe. "
        "Responde siempre en español de forma formal y precisa. "
        "Utiliza EXCLUSIVAMENTE la siguiente base de conocimiento para responder. "
        f"Base de conocimiento: {json.dumps(base_conocimiento, ensure_ascii=False)}"
    )

    # 3. Envía la pregunta a Google Gemini
    respuesta = cliente.models.generate_content(
        model="gemini-flash-latest",
        contents=pregunta_del_usuario,
        config={
...
if __name__ == "__main__":
    print("Probando conexión con el asistente...")
    respuesta = consultar_asistente("¿Qué servicios o asesorías ofrece Construcción Segura?")
